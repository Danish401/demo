...
/** Map BOQ_Details row (Normal type) to template BOQ field names */
function mapBoqDetailsRow(row: Record<string, unknown>): DoorCoreBOQItem {
  const v = (key: string) => row[key] as DoorCoreBOQItem[keyof DoorCoreBOQItem]
  return {
    S_No1: v('S_No'),
    Door_Ref: v('Door_Ref') as string | undefined,
    Door_Leaf_Width_mm: v('Door_Leaf_Width_mm'),
    Door_Leaf_Height_mm: v('Door_Leaf_Height_mm'),
    Leaf_Thick_mm: v('Leaf_Thick_mm'),
    Door_Type: v('Door_Type') as string | undefined,
    Product_Ref: v('Product_Ref') as string | undefined,
    Acoustic_Rating_db: v('Acoustic_Rating_db'),
    Fire_Rating_mins: v('Fire_Rating_mins'),
    Vision_Panel_mm: v('Vision_Panel_mm') as string | undefined,
    Qty1: v('Qty'),
...
/** Normalize Door Core records (Quotation_Log_Door_Core / Core_Cover_page_Report): BOQ subform by Type_field + trader name */
function normalizeCoreCoverPageRecord(raw: Record<string, unknown>): CoreCoverPageData {
  ...
  const typeField = String(pick(['Type_field', 'type_field']) ?? '').trim()
  const isNormal = typeField.toLowerCase() === 'normal'

  let BOQ: DoorCoreBOQItem[] = []
  if (isNormal) {
    const details = pick(['BOQ_Details', 'boq_details'])
    BOQ = Array.isArray(details)
      ? details.map((row) => mapBoqDetailsRow(row as Record<string, unknown>))
      : []
  } else {
    const tenderBoq = pick(['BOQ', 'boq'])
    BOQ = Array.isArray(tenderBoq) ? (tenderBoq as DoorCoreBOQItem[]) : []
  }
  ...
  const traderName1 =
    (pick(['Trader_Name1', 'trader_name1']) as string | undefined) ?? (raw.Trader_Name1 as string | undefined)
  const excelUploadRaw = pick(['Excel_Upload1', 'excel_upload1'])
  return {
    ...raw,
    ID: String(raw.ID ?? ''),
  ...
  return (
    <main className="quotation-doc" style={{ padding: '16px' }}>
      ...
      {!loading && !error && !fitoutData && !doorSet1Data && !doorSet2Data && doorCoreData && (
        <DoorCoreQuotationContent
          data={doorCoreData}
          viewMode={viewMode}
          reportName={(searchParams.get('report') || 'Quotation_Log_Door_Core').trim()}
        />
      )}
