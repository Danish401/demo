...
async function fetchDoorCoreReport(accessToken: string, searchParams: URLSearchParams): Promise<Response> {
  ...
  // Quotation_Log_Door_Core / Core_Cover_page_Report: Tender uses BOQ; Normal uses BOQ_Details; Tender attachment Excel_Upload1
  const DOOR_CORE_SUBFORMS = 'BOQ,BOQ_Details,Type_field,Excel_Upload1'
  if (reportLinkName === 'Quotation_Log_Door_Core' || reportLinkName === 'Core_Cover_page_Report') {
    fields = fields ? `${fields},${DOOR_CORE_SUBFORMS}` : DOOR_CORE_SUBFORMS
  }
